...
class Articlecategory extends Admin
{
    ...
    public function index($id = null)
    {
        ...
        // 数据列表
        $data_list = Db::view('cms_articlecategory', true)
            ->order('cms_articlecategory.id')
            ->select();
    ...
    public function add($pid = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($Category = ArticlecategoryModel::create($data)) {
                // 记录行为
        ...
        return ZBuilder::make('form')
            ->addFormItems([
                ['hidden', 'pid', $pid],
                ['select', 'pid', '父类', '<code>必选</code>', ArticlecategoryModel::getTreeList(),$pid],
                ['text', 'title', '分类标题'],
                ['text', 'sort', '排序', '', 100],
                ['image', 'cover', '分类图片'],
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (ArticlecategoryModel::update($data)) {
                // 记录行为
        ...
        return ZBuilder::make('form')
            ->addFormItems([
                ['hidden', 'id'],
                ['select', 'pid', '父类', '<code>必选</code>', ArticlecategoryModel::getTreeList(0)],
                ['text', 'title', '分类标题'],
                ['text', 'sort', '排序', '', 100],
                ['image', 'cover', '分类图片'],
                ['radio', 'status', '立即启用', '', ['否', '是'], 1]
            ])
            ->setFormData(ArticlecategoryModel::get($id))
            ->fetch();
    }
    ...
    {
        // 检查是否有子菜单
        if (ArticlecategoryModel::where('pid', $ids)->find()) {
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $menu_title = ArticlecategoryModel::where('id', 'in', $ids)->column('title');
    ...
    public function quickEdit($record = [])
    {
        ...
        $menu    = ArticlecategoryModel::where('id', $id)->value($field);
