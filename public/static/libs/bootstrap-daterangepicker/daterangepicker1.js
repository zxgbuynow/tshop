...
(function(root, factory) {
    ...
    DateRangePicker.prototype = {
        ...
        renderCalendar: function(side) {
            ...
            /*myAdd start*/
            var yearMonthArr = dateHtml.split('月');
            var myVar = {
                lunar: null,
                // classStr:cname.replace(/^\s+|\s+$/g, ''),
                year: parseInt(yearMonthArr[1]),
                month: parseInt(yearMonthArr[0]),
                day: 1,
                lunarDay: null,
                startDate: this.startDate.format('YYYY-MM-DD').split('-'),
                //endDate:('endDate' in this)?this.endDate.format('YYYY-MM-DD').split('-'):null,
                lunarMonth: ''
            }
            if (this.endDate == null && myVar.startDate[1] == myVar.month && myVar.startDate[0] == myVar.year) {
                // console.log('no endDate')
                myVar.lunar = myCalendar.solar2lunar(myVar.startDate[0], myVar.startDate[1], myVar.startDate[2]);
                myVar.lunarMonth = '（农历 ' + myVar.lunar.IMonthCn + '）';
            } else if (this.endDate !== null) {
                //console.log('has endDate')
                myVar.endDate = this.endDate.format('YYYY-MM-DD').split('-');
                if (side == 'left' && myVar.startDate[1] == myVar.month && myVar.startDate[0] == myVar.year) {
                    //console.log('left')
                    myVar.lunar = myCalendar.solar2lunar(myVar.startDate[0], myVar.startDate[1], myVar.startDate[2]);
                    myVar.lunarMonth = '（农历 ' + myVar.lunar.IMonthCn + '）';
                } else if (side == 'right' && myVar.endDate[1] == myVar.month && myVar.endDate[0] == myVar.year) {
                    //console.log('right')
                    myVar.lunar = myCalendar.solar2lunar(myVar.endDate[0], myVar.endDate[1], myVar.endDate[2]);
                    myVar.lunarMonth = '（农历 ' + myVar.lunar.IMonthCn + '）';
                }
            }
            // console.log(myVar.lunar);
            /*myAdd end*/
            /*myChange start*/
            html += '<th colspan="5" class="month">' + dateHtml + myVar.lunarMonth + '</th>'; //（农历 '+ myVar.lunar.IMonthCn +'）
            /*myChange end*/
            ...
            for (var row = 0; row < 6; row++) {
                html += '<tr>';
                // add week number
                if (this.showWeekNumbers) html += '<td class="week" weekDateData=' + calendar[row][0].year() + '-' + (calendar[row][0].month() + 1) + '-' + calendar[row][0].date() + '>' + calendar[row][0].week() + '</td>';
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    /*myAdd start*/
                    myVar.year = parseInt(calendar[row][col].format("YYYY"));
                    myVar.month = parseInt(calendar[row][col].month()) + 1;
                    myVar.day = calendar[row][col].date();
                    myVar.lunar = myCalendar.solar2lunar(myVar.year, myVar.month, myVar.day);
                    myVar.lunarDay = myVar.lunar.IDayCn;
                    myVar.lunarDayClass = 'lunarDay';
                    // console.log(myVar.lunar)
                    if (myVar.lunar.Term || myVar.lunar.lunarFestivals || myVar.lunar.calendarFestivals) {
                        myVar.lunarDay = myVar.lunar.lunarFestivals || myVar.lunar.calendarFestivals || myVar.lunar.Term;
                        myVar.lunarDayClass += ' lunarTerm';
                    }
                    /*myAdd end*/
                    html += '<td class="' + cname.replace(/^\s+|\s+$/g, '') + '" data-title="' + 'r' + row + 'c' + col + '">' + calendar[row][col].date() + '<br /><span class="' + myVar.lunarDayClass + '">' + myVar.lunarDay + '</span></td>';
                }
                html += '</tr>';
            }
            html += '</tbody>';
