...
<div class="form-group  col-xs-12 <?php echo (isset($extra_class) && ($extra_class !== '')?$extra_class:''); ?>" id="form_group_<?php echo $name; ?>">
    ...
    <div class="col-sm-12">
        <?php if(is_array($form[type].options) || $form[type].options instanceof \think\Collection || $form[type].options instanceof \think\Paginator): $i = 0; $__LIST__ = $form[type].options;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$option): $mod = ($i % 2 );++$i;?>
        ...
        <?php endforeach; endif; else: echo "" ;endif; if(!(empty($tips) || (($tips instanceof \think\Collection || $tips instanceof \think\Paginator ) && $tips->isEmpty()))): ?>
